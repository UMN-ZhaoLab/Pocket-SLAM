"""Short paper-aligned validation on EuRoC V2_01_easy (IGEV depth).

Paper tables use MH01–MH05; this run checks that the fixed pipeline
(IGEV depth + paper hyperparams + per-frame mapping) produces sane ATE/PSNR
instead of the broken SGBM ~8 m ATE regime.
"""
primary_device = "cuda:0"
seed = 0
scene_name = "V2_01_easy_val"
...
# 300-frame subset, stride 5 → ~55 keyframes after pose sync
start_idx = 0
end_idx = 275
stride = 5

group_name = "euroc_paper_val"
...
config = dict(
    ...
    pocket_slam=dict(
        enable=True,
        N_tar_ratio=0.7,          # keep more than paper 0.4 to close PSNR gap
        N_tar=None,
        B_min=5,
        B_max=200,
        tile_size=16,
        global_cap=False,
        protect_age=0,            # paper: only current-frame new Gaussians
        warmup_frames=1,
        prune_interval=1,
        prune_margin=1.0,
        score_opacity_weight=0.0, # paper rendering-area score
        post_prune_iters=30,      # brief remap after prune to heal holes
    ),
    viz=dict(
